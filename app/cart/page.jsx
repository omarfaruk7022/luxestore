...
export default function CartPage() {
  const { cart, fetchCart, updateItem, removeItem, itemCount, subtotal, isLoading } = useCartStore();
  ...
  if (!isAuthenticated()) return (
    <div className="min-h-screen pt-24 flex items-center justify-center">
      <div className="text-center">
        ...
        <h2 className="font-display text-3xl font-light mb-2">Sign in to view your cart</h2>
        <Link href="/login" className="mt-4 inline-block px-6 py-3 rounded-xl bg-foreground text-background text-sm font-medium">Sign In</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          {itemCount() > 0 && (
            <span className="px-3 py-1 rounded-full bg-secondary text-sm font-medium">{itemCount()} items</span>
          )}
        </div>
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <Link href="/shop" className="flex items-center gap-2 px-8 py-3.5 rounded-xl bg-foreground text-background font-medium hover:opacity-90 transition-opacity">
            ...
            <div className="space-y-4">
                ...
                {shippingFree ? (
                  ...
                  <>
                    <p className="text-sm text-muted-foreground mb-2">
                      Add <span className="font-medium text-foreground">{formatPrice(SHIPPING_THRESHOLD - subtotal())}</span> more for free shipping
                    </p>
              ...
              <AnimatePresence>
                {items.map((item) => (
                  ...
                  >
                    <Link href={`/shop/${item.product?.slug || '#'}`} className="w-24 h-24 rounded-xl overflow-hidden bg-secondary shrink-0 block">
                      {item.image && (
                        <Image src={item.image} alt={item.name} width={96} height={96} className="w-full h-full object-cover hover:scale-105 transition-transform" />
                      )}
                    </Link>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <Link href={`/shop/${item.product?.slug || '#'}`} className="font-medium hover:text-accent transition-colors line-clamp-2">
                            {item.name}
                          </Link>
                          <div className="flex items-center gap-3 mt-1.5">
                            <span className="text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">{item.size}</span>
                            <span className="flex items-center gap-1 text-xs text-muted-foreground">
                              <span className="w-3 h-3 rounded-full border border-border inline-block" style={{ background: item.colorHex }} />
                              {item.color}
                            </span>
                          </div>
                        </div>
              ...
              <div className="p-4 rounded-2xl border bg-card">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Visa', 'Mastercard', 'bKash', 'Nagad', 'COD'].map((method) => (
                    <span key={method} className="px-2.5 py-1 text-xs border rounded-lg font-medium">{method}</span>
                  ))}
                </div>
              </div>
            </div>
