...
import { Plus, Edit2, Trash2, Search, ChevronDown, X, Check, Star, Package } from 'lucide-react';
...
const EMPTY_FORM = {
  ...
  category: '', images: [''], material: '', brand: 'LuxeWear', tags: '',
...
export default function AdminProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productApi.getAll({ page, limit: 10, search: search || undefined }).then((r) => r.data),
  });
    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          {pagination && <p className="text-muted-foreground text-sm mt-1">{pagination.total} products</p>}
        </div>
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b bg-secondary/30">
              <tr>
                {['Product', 'Category', 'Price', 'Stock', 'Rating', 'Sold', 'Badges', 'Actions'].map((h) => (
                ...
                Array(8).fill(null).map((_, i) => (
                  <tr key={i}>
                    {Array(8).fill(null).map((_, j) => (
                      <td key={j} className="py-4 px-3 md:px-4"><div className="h-4 rounded shimmer" /></td>
                    ))}
                  </tr>
                ))
                  ...
                  <td className="py-3 px-3 md:px-4 first:pl-4 md:first:pl-6">
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium truncate max-w-[120px] md:max-w-[180px]">{product.name}</p>
                        <p className="text-[10px] md:text-xs text-muted-foreground font-mono truncate max-w-[120px] md:max-w-none">{product.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-3 md:px-4 text-muted-foreground whitespace-nowrap text-[11px] md:text-sm">{product.category?.name}</td>
                  <td className="py-3 px-3 md:px-4 whitespace-nowrap">
                    <p className="font-semibold text-[11px] md:text-sm">{formatPrice(product.discountPrice || product.price)}</p>
                    {product.discountPrice && <p className="text-[10px] md:text-xs text-muted-foreground line-through">{formatPrice(product.price)}</p>}
                  </td>
                  <td className="py-3 px-3 md:px-4 whitespace-nowrap">
                    <span className={cn('font-medium text-[11px] md:text-sm', product.totalStock === 0 ? 'text-destructive' : product.totalStock < 10 ? 'text-amber-500' : '')}>
                      {product.totalStock}
                    </span>
                  </td>
                  <td className="py-3 px-3 md:px-4 whitespace-nowrap">
                    ...
                    ) : <span className="text-muted-foreground text-[11px] md:text-sm">—</span>}
                  </td>
                  <td className="py-3 px-3 md:px-4 text-muted-foreground text-[11px] md:text-sm whitespace-nowrap">{product.sold}</td>
                  <td className="py-3 px-3 md:px-4 min-w-[120px]">
                    <div className="flex flex-wrap gap-1">
                      {product.isFeatured && <span className="px-1.5 py-0.5 text-[9px] md:text-[10px] rounded bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300">Featured</span>}
                      {product.isNewArrival && <span className="px-1.5 py-0.5 text-[9px] md:text-[10px] rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">New</span>}
                      {product.isBestSeller && <span className="px-1.5 py-0.5 text-[9px] md:text-[10px] rounded bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">Best</span>}
                    </div>
                  </td>
                  <td className="py-3 px-3 md:px-4 last:pr-4 md:last:pr-6 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                       <button onClick={() => openEdit(product)} className="p-1.5 rounded-lg hover:bg-secondary transition-colors">
                        <Edit2 size={14} className="md:w-4 md:h-4 w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDelete(product._id)} className="p-1.5 rounded-lg hover:bg-destructive/10 hover:text-destructive transition-colors">
                        <Trash2 size={14} className="md:w-4 md:h-4 w-3.5 h-3.5" />
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            className="px-4 py-2 rounded-xl border text-sm disabled:opacity-40 hover:bg-secondary transition-colors">Previous</button>
          <span className="text-sm text-muted-foreground">Page {page} of {pagination.pages}</span>
          <button onClick={() => setPage(p => Math.min(pagination.pages, p + 1))} disabled={page === pagination.pages}
            className="px-4 py-2 rounded-xl border text-sm disabled:opacity-40 hover:bg-secondary transition-colors">Next</button>
        </div>
      )}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b sticky top-0 bg-card z-10">
                ...
                <button onClick={() => setShowForm(false)} className="p-2 rounded-full hover:bg-secondary transition-colors">
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <input value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} placeholder="cotton, comfort, everyday"
                      className="w-full px-4 py-2.5 rounded-xl border bg-background text-sm outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                  ...
                  {/* Badges */}
                  <div className="col-span-2">
                    <label className="text-sm font-medium mb-3 block">Product Badges</label>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button type="button"
                        onClick={() => setForm({ ...form, variants: [...form.variants, { size: 'M', color: 'Black', colorHex: '#000000', stock: 10 }] })}
                        className="text-xs text-accent hover:underline flex items-center gap-1">
                      ...
                      {form.variants.map((v, i) => (
                        <div key={i} className="grid grid-cols-5 gap-2 items-center">
                          <input value={v.size} onChange={(e) => updateVariant(i, 'size', e.target.value)} placeholder="Size"
                            className="px-3 py-2 rounded-xl border bg-background text-sm outline-none" />
                          ...
                          <input type="color" value={v.colorHex} onChange={(e) => updateVariant(i, 'colorHex', e.target.value)}
                            className="h-10 px-2 rounded-xl border bg-background cursor-pointer w-full" />
                          ...
                          <button type="button" onClick={() => setForm({ ...form, variants: form.variants.filter((_, j) => j !== i) })}
                            className="p-2 rounded-xl border hover:bg-destructive/10 hover:text-destructive transition-colors">
                            <X size={14} />
                          </button>
                        </div>
                      ))}
