      ...
      <Link href={`/shop/${product.slug}`}>
        ...
        <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-secondary mb-4">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {discount > 0 && (
              <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-accent text-white">-{discount}%</span>
            )}
            {product.isNewArrival && (
              <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-foreground text-background">New</span>
            )}
            {product.isBestSeller && (
              <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-secondary border">Best Seller</span>
            )}
          </div>
          ...
          <button
            ...
            className={cn(
              ...
              'bg-background/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 hover:bg-background',
          ...
          {product.totalStock === 0 && (
            <div className="absolute inset-0 bg-background/60 flex items-center justify-center">
              <span className="px-3 py-1.5 rounded-full bg-background border text-xs font-medium">Out of Stock</span>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-2">
              <span className="font-semibold">{formatPrice(product.discountPrice || product.price)}</span>
